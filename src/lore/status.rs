...
///
/// A thread reply saying "Merged, thanks" wins (green); otherwise a
...
pub fn compute_status(emails: &[Email]) -> PatchStatus {
    let mut reviewed = false;
    for email in emails {
        if body_has_merged(&email.body) {
...
fn body_has_merged(body: &str) -> bool {
    body.lines()
        .filter(|line| !is_quoted(line))
        .any(|line| line.to_ascii_lowercase().contains("merged, thanks"))
}
...
mod tests {
    ...
    fn normal_when_no_tags() {
        ...
        assert_eq!(compute_status(&emails), PatchStatus::Normal);
    }

    #[test]
    fn reviewed_when_reply_has_trailer() {
        let emails = vec![email("Please review.\n"), email("Reviewed-by: Bob <b@x>\n")];
        assert_eq!(compute_status(&emails), PatchStatus::Reviewed);
    }
    ...
    fn merged_wins_over_reviewed() {
        ...
        assert_eq!(compute_status(&emails), PatchStatus::Merged);
    }
